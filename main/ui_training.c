...
void ui_training_init(void)
{
    ...
    // -------------------------------------------------------------------------
    // Black line (2px) below CONT_MID - Visual bottom line
    // -------------------------------------------------------------------------
    lv_obj_t *line_bottom_mid = lv_obj_create(scr);
    lv_obj_set_size(line_bottom_mid, 2, 480);
    ...
    // -------------------------------------------------------------------------
    // CONT_GREY_RIGHT (Right Grey Block - Visual Right): X=449, 128x90. Dark Grey.
    // -------------------------------------------------------------------------
    lv_obj_t *cont_grey_right = lv_obj_create(scr);
    lv_obj_set_size(cont_grey_right, 128, 90);
    lv_obj_set_pos(cont_grey_right, 449, 0);
    ...
    lv_obj_set_style_text_font(lbl_delantero, &lv_font_montserrat_36, LV_PART_MAIN);
    ...
    // -------------------------------------------------------------------------
    // CONT_WHITE (White Strip): X=449, 128x300. White.
    // -------------------------------------------------------------------------
    lv_obj_t *cont_white = lv_obj_create(scr);
    lv_obj_set_size(cont_white, 128, 300);
    lv_obj_set_pos(cont_white, 449, 90);
    ...
    // -------------------------------------------------------------------------
    // CONT_GREY_LEFT (Left Grey Block - Visual Left): X=449, 128x90. Dark Grey.
    // -------------------------------------------------------------------------
    lv_obj_t *cont_grey_left = lv_obj_create(scr);
    lv_obj_set_size(cont_grey_left, 128, 90);
    lv_obj_set_pos(cont_grey_left, 449, 390);
    ...
    lv_obj_align(img_trasero, LV_ALIGN_CENTER, -34, -2); // -34 X = 34px up (visual), -2 Y = 13px left (visual)

    // Label "3/11" below trasero icon
    lv_obj_t *lbl_trasero = lv_label_create(cont_grey_left);
    lv_label_set_text(lbl_trasero, "3/11");
    lv_obj_set_style_text_font(lbl_trasero, &lv_font_montserrat_36, LV_PART_MAIN);
