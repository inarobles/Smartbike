...
{
    lv_event_code_t code = lv_event_get_code(e);
    ESP_LOGI("UI_CARDIO", "Back button event code: %d", (int)code);
    ...
    if(code == LV_EVENT_CLICKED || code == LV_EVENT_RELEASED) {
        ...
        if ((esp_timer_get_time() / 1000 - s_enter_time) < 500) {
            ESP_LOGW("UI_CARDIO", "Back click ignored (debounce)");
            return;
        }

        ESP_LOGI("UI_CARDIO", "Going back to settings");
        audio_manager_play_event(AUDIO_EVENT_BUTTON);
...
{
    ESP_LOGI("UI_CARDIO", "Initializing Cardio Screen");
